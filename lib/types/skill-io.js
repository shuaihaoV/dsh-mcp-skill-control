...
export function resolveSkillRoots(env = process.env) {
    ...
    for (const { source, dir } of logical) {
        ...
        if (!isDirectory(physical))
            continue;
        const sources = merged.get(physical);
        if (sources === undefined)
            merged.set(physical, [source]);
 ...
 */
export function setModelDisabled(path, disabled) {
    let text;
    try {
        text = readFileSync(path, 'utf8');
    }
    ...
    if (disabled) {
        ...
        if (current !== true && !fm.has(DISABLE_KEY))
            return false;
        if (!fm.has(DISABLE_KEY))
            return false;
 ...
 */
export function resolveManagedSkillPath(path, env = process.env) {
...
//# sourceMappingURL=skill-io.js.map