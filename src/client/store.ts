...
const EMPTY: McpSnapshot = { read: false, rows: [], busy: {}, adding: false }
 ...
 */
export function createMcpInventory(port: McpPort, onError: (error: unknown) => void): McpInventory {
  let snapshot: McpSnapshot = EMPTY
  ...
  const flags = (): Pick<McpSnapshot, 'busy' | 'adding'> => ({
    busy: Object.fromEntries([...busy].map(id => [id, true])),
    adding: adding > 0,
  })

  const set = (next: Omit<McpSnapshot, 'busy' | 'adding'>): void => {
    snapshot = { ...next, ...flags() }
    emit()
  }
  ...
  const failAction = (message: string): void => {
    set({
      read: snapshot.read,
      rows: snapshot.rows,
  ...
  async function act(entryId: string, op: () => Promise<McpActionResult>): Promise<McpActionResult> {
    busy.add(entryId)
    touch()
    try {
  ...
  return {
    ...
    clearActionError() {
      if (snapshot.actionError === undefined) return
      set({
        read: snapshot.read,
        rows: snapshot.rows,
        ...snapshot.error === undefined ? {} : { error: snapshot.error },
...
const SKILL_EMPTY: SkillSnapshot = { read: false, rows: [], busy: {} }
 ...
 */
export function createSkillInventory(
  port: McpPort,
  onActionError: (message: string) => void,
): SkillInventory {
  let snapshot: SkillSnapshot = SKILL_EMPTY
  const listeners = new Set<() => void>()
  const busy = new Set<string>()
  ...
  const set = (next: Omit<SkillSnapshot, 'busy'>): void => {
  ...
  async function setDisabled(path: string, disabled: boolean): Promise<SkillActionResult> {
    busy.add(path)
    touch()
    try {
