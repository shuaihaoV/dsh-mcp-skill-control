...
import type { McpReconnectSpec, McpServerSpec } from '../types.ts'
...
export function parseEnv(text: string): { values: Record<string, string>; bad: string[] } {
...
export function parseHeaders(text: string): { values: Record<string, string>; bad: string[] } {
  const values: Record<string, string> = {}
  const bad: string[] = []
  for (const raw of text.split('\n')) {
    const line = raw.trim()
...
export function formatPairs(values: Record<string, string>, separator: string): string {
...
function reconnectOf(value: unknown): McpReconnectSpec | undefined {
  if (!isRecord(value)) return undefined
  const out: McpReconnectSpec = {}
 ...
 */
export function candidateFrom(name: string, raw: unknown): ImportCandidate {
  if (!isRecord(raw)) return { name, problem: 'not a JSON object' }
...
export function parseImport(text: string): ImportResult {
  const trimmed = text.trim()
  if (trimmed === '') return { candidates: [], fatal: 'paste a JSON document first' }
  let parsed: unknown
  try {
  ...
  if (!isRecord(parsed)) return { candidates: [], fatal: 'the document root must be a JSON object' }
